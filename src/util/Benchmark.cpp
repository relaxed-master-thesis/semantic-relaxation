...
void Benchmark::run() {
	for (auto &executor : executors) {
		...
		if (!cumRes.isValid) {
			std::cerr << execName << " failed to produce result, skipping...\n";
...
void Benchmark::printResults() {
	struct StrResults {
		StrResults(std::string &name, long double mean, uint64_t max, long tot,
				   float tot_speedup, long calc, float calc_speedup, long prep,
				   float prep_speedup, BenchmarkType type)
	...
	for (size_t i = 0; i < executors.size(); ++i) {
		...
		sresults.emplace_back(name, mean, max, tot, tot_spedup, calc,
							  calc_speedup, prep, prep_speedup, exec->type());
	}
	...
	for (size_t i = 0; i < results.size(); ++i) {
		...
		if (res.type == BenchmarkType::Approximate) {
			float mean_diff =
				std::abs(correct_mean - ares.measurement.mean) / correct_mean;
			float max_diff =

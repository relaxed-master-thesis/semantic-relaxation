...
AbstractExecutor::Measurement FAAImp::calcMaxMeanError() { return {0, 0}; }
void FAAImp::reset() {
	puts = nullptr;
	gets = nullptr;
}
...
AbstractExecutor::Measurement IMEImp::execute() {
	...
	for (size_t i = 0; i < intervals.size(); ++i) {
		...
		long double rank =
			(((long double)intv_avg_size / i_size) * i) - interval.pop_idx;
		rank = rank < 0.0f ? 0.0f : rank;
		rank =
			i < interval.pop_idx ? interval.pop_idx - i : i - interval.pop_idx;
...
AbstractExecutor::Measurement IMEImp::calcMaxMeanError() { return {0, 0}; }
void IMEImp::reset() {
	intervals.clear();
	intv_avg_size = 0;
	put_count = 0;
